...
class TargetAudienceInsights(BaseModel):
    ...
    def find_audience(prod_name, generated_description, text: Optional[str] = None,pictures: list = None):
        ...
        messages.append({"role": "user", "content": f"Product name: {prod_name}"})
        messages.append({"role": "user", "content": f"Product description generated by us (low priority): {generated_description}"})
        if text:
            messages.append({"role": "user", "content": f"Product description generated by user (high priority): {text}"})
...
class ProductInformation(BaseModel):
    ...
    def process_Images(self) -> list:
        ...
        for image in self.product_images_url:
            img_inputs.append(utils.ImageToText(image))
    ...
    def find_product_information(text: Optional[str] = None, voice: Optional[str] = None, pictures: Optional[str] = None, videos: Optional[str] = None):
        ...
        if pictures:
            for image in pictures:
                messages.append({"role": "user", "content": f"The following pictures provide visual context for the product:{image}"})"})
